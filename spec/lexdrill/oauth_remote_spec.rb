...
RSpec.describe Lexdrill::OauthRemote do
  around do |example|
    Dir.mktmpdir("lexdrill-oauth-remote-spec") do |dir|
      @path = File.join(dir, ".drill.oauth-remote")
      example.run
    end
  end

  before { stub_const("Lexdrill::OauthRemote::PATH", @path) }
